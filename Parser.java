...
// 6/12/2023
// Project 1
// Creates and defines the Parser class to read and process tokens and their expected details
...
class Parser {
    ...
    private void parseImages(Scene scene, Token imageToken) throws LexicalError, SyntaxError, IOException {
        ...
        if (imageToken == Token.RIGHT_TRIANGLE) {
            ...
            verifyNextToken(Token.NUMBER);
            width = lexer.getNumber();
            IsoscelesTriangle iso = new IsoscelesTriangle(color, point, height, width);
            scene.addImage(iso);
        } else if (imageToken == Token.PARALLELOGRAM) {
            location = getNumberList(2);
            Point point2 = new Point(location[0], location[1]);
            verifyNextToken(Token.OFFSET);
            verifyNextToken(Token.NUMBER);
            offset = lexer.getNumber();
            Parallelogram para = new Parallelogram(color, point, point2, offset);
            scene.addImage(para);
            ...
            RegularPolygon reg = new RegularPolygon(color, point, sides, radius);
            scene.addImage(reg);
        } else if (imageToken == Token.TEXT) {
            verifyNextToken(Token.STRING);
            Text str = new Text(color, point, lexer.getLexeme());
            scene.addImage(str);
        } else {
